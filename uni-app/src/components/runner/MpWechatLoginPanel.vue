<template>
  <view class="wx-login-body">
    ...
    <view v-if="eventName" class="wx-login-event">
      <text class="wx-login-event-label">当前赛事</text>
      <text class="wx-login-event-name">{{ eventName }}</text>
    </view>
    ...
    <template v-else>
      ...
      <text v-if="showSkip" class="wx-login-skip" @tap="onSkip">暂不登录，先逛逛</text>
    </template>
  </view>
</template>
...
import {
  getStoredWxProfile,
  getMpPrivacySetting,
  openMpPrivacyContract,
} from '@/utils/mp-login.js'

const props = defineProps({
  loading: Boolean,
  showSkip: { type: Boolean, default: true },
  title: { type: String, default: '授权微信个人信息' },
  desc: {
    type: String,
    default: '用于展示您的微信昵称与头像，便于赛事助手识别您。不会获取手机号。',
  },
...
const emit = defineEmits(['authorize', 'skip'])
...
defineExpose({ setError, initPrivacy })
</script>
